...
- `lib/app/version_info.dart`：阅读页版本标签常量（每次发布后更新展示 tag）。
...
- `lib/features/reader/presentation/pages/reader_page.dart`：阅读主页面实现（P0-01~P0-04）：
  ...
  - 静态伪装聊天区（头像+时间戳+双向消息）
  - 右上角 `···` 菜单及入口跳转
  - 左上角版本 tag 展示（来自 `version_info.dart`）
...
- `docs/p0/05-实施清单_工1_P0-11到P0-13.md`：工1进度保存与恢复实施文档（P0-11/12/13）。
